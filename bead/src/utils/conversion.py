...
def process_event(evt_id, row):
    ...
    for i in range(num_jets):
        ...
        jets.append([
            evt_id, i, num_constits, b_tagged,
            jet_properties['jet_pt'], jet_properties['jet_eta'], jet_properties['jet_phi']
        ])
...
def convert_csv_to_hdf5_npy_parallel(csv_file, output_prefix, file_type='h5', output_path, n_workers=4, verbose: bool = False):
    """
    ...
    with get_reusable_executor(max_workers=n_workers) as executor:
        futures = [executor.submit(process_event, evt_id-1, row) for evt_id, row in enumerate(data.itertuples(), start=1)]
