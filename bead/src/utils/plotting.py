...
def plot_latent_variables(config, paths, verbose=False):
    ...
    label_file = os.path.join(paths["output_path"], "results", config.input_level + "_label.npy")
    gen_label_file = os.path.join(paths["data_path"], config.file_type, "tensors", "processed", "gen_label_"+config.input_level+".npy")
    ...
    if len(gen_labels) != n_background:
        raise ValueError(
            "Mismatch: gen_label size ({}) does not match number of background samples in label.npy ({})"
            .format(len(gen_labels), n_background)
        )
    if verbose:
        print("Loaded {} total samples: {} background and {} signal samples."
              .format(len(labels), n_background, len(labels) - n_background))
    ...
    def reduce_dim(data):
        ...
        if config.latent_space_size > 50:
            if verbose:
                print("Applying PCA to reduce latent space from {} to 50 dimensions..."
                      .format(config.latent_space_size))
        ...
        if style == 'pca':
            ...
            raise ValueError("Invalid latent_space_plot_style: {}. Must be 'pca', 'tsne', or 'trimap'."
                             .format(config.latent_space_plot_style))
    ...
    for latent_data, suffix in [(z0, "_z0"), (zk, "_zk")]:
        ...
        plt.title("{} embedding using {}".format(suffix[1:].upper(), method_used.upper()))
        ...
        save_path = os.path.join(paths["output_path"], "plots", "latent_space", "{}{}.pdf".format(config.project_name, suffix))
...
def plot_mu_logvar(config, paths, verbose=False):
    ...
    n_background = np.sum(labels == 0)
    if len(gen_labels) != n_background:
        raise ValueError("Mismatch: gen_label size ({}) does not match number of background samples in label.npy ({})".format(
            len(gen_labels), n_background))
    if verbose:
        print("Loaded {} total samples: {} background and {} signal samples.".format(
            len(labels), n_background, len(labels) - n_background))
        print("Loaded mu_data with shape {} and logvar_data with shape {}.".format(mu.shape, logvar.shape))
    ...
    def reduce_dim(data):
        ...
        if config.latent_space_size > 50:
            if verbose:
                print("Performing initial PCA to reduce from {} to 50 dimensions.".format(config.latent_space_size))
            data = PCA(n_components=50).fit_transform(data)
        ...
        if style == 'pca':
            ...
            raise ValueError("Invalid latent_space_plot_style: {}. Choose from 'pca', 'tsne', or 'trimap'.".format(config.latent_space_plot_style))
        return reduced, method_used
    ...
    plt.figure(figsize=(8, 6))
    plt.scatter(reduced_mu[:, 0], reduced_mu[:, 1],
                c=colors, alpha=0.7, edgecolors='w', s=60)
    ...
    mu_save_path = os.path.join(paths["output_path"], "plots", "latent_space", "{}_mu.pdf".format(config.project_name))
    ...
    plt.figure(figsize=(8, 6))
    bins = 30
    plt.hist(uncertainty_herwig, bins=bins, color='green', alpha=0.6, label='Herwig (gen_label 0)')
    plt.hist(uncertainty_pythia, bins=bins, color='blue', alpha=0.6, label='Pythia (gen_label 1)')
    plt.hist(uncertainty_sherpa, bins=bins, color='yellow', alpha=0.6, label='Sherpa (gen_label 2)')
    ...
    uncertainty_save_path = os.path.join(paths["output_path"], "plots", "latent_space", "{}_uncertainty.pdf".format(config.project_name))
...
def plot_roc_curve(config, paths, verbose: bool = False):
    ...
    label_path = os.path.join(paths["output_path"], "results", config.input_level + "_label.npy")
    ...
    for component in loss_components:
        ...
        if len(data) != len(ground_truth):
            raise ValueError(f"Length mismatch: {file_path} has {len(data)} entries; "
