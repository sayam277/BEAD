...
def load_data(file_path, file_type="h5", verbose: bool = False):
    ...
    if file_type == "h5":
        ...
        raise NotImplementedError("Loading .npy files is not yet supported. Please retry with --mode = convert_csv and --options = h5 first.")
    else:
        raise ValueError("Unsupported file type. First convert to 'h5' or 'npy' using --mode = convert_csv and --options = [chosen file_type].")
...
def decode_pid(encoded_tensor_path, pid_map_path, decoded_tensor_path):
    ...
    encoded_to_original = dict(zip(pid_map_df["encoded_pid"], pid_map_df["original_pid"]))
...
def process_event(evt_id, evt_jets, evt_constits, n_jets=3, n_constits=15, verbose: bool = False):
    """
    Process a single event to select top jets and their top constituents.
    """
    
    if verbose:
        print(f"Processing event {evt_id} with {len(evt_jets)} jets and {len(evt_constits)} constituents...")
    ...
    selected_constits = np.zeros((n_jets * n_constits, evt_constits.shape[1]), dtype=evt_constits.dtype)
    ...
    for jet_idx in range(min(len(evt_jets), n_jets)):
        ...
        jet_constits[:,3] = jet_btag  # Replace constituent pid with btag info of corresponding jet
...
def parallel_select_top_jets_and_constituents(jets, constituents, n_jets=3, n_constits=15, n_workers=4, verbose: bool = False):
    """
    Parallelized selection of top jets and constituents.
    """
    
    if verbose:
        print(f"Selecting top {n_jets} jets and their top {n_constits} constituents in parallel...")
    ...
    if verbose:
        print(f"Processing {len(event_data)} events in parallel using {n_workers} workers...")
    jet_results = []
    constits_results = []
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = [executor.submit(process_event, *event, n_jets, n_constits) for event in event_data]
    ...
    if verbose:
        print(f"shape of jet_selection: {jet_selection.shape}\nshape of constits_selection: {constits_selection.shape}")
    
    return jet_selection, constits_selection


def process_and_save_tensors(input_path, out_path, output_prefix, config, verbose: bool =False):
    """
    ...
    if verbose:
        print(f"Processing {n_jets} jets and {n_constits} constituents from {input_path}...")
    
    # Load the data
    events, jets, constituents = load_data(input_path, file_type, verbose)
    if verbose:
        print(f"Loaded {len(events)} events, {len(jets)} jets, and {len(constituents)} constituents from {input_path}")
    ...
    if norm:
        ...
        if norm == "pj_custom":
            ...
            constituents_norm, constit_comp_scaler = normalization.normalize_constit_pj_custom(constituents)
        else:
            jets_norm, jet_comp_scaler = helper.normalize_data(jets, norm)
            constituents_norm, constit_comp_scaler = helper.normalize_data(constituents, norm)
        if verbose:
            print("Normalization complete.")
            print(f"Jets shape after normalization: {jets_norm.shape}\nConstituents shape after normalization: {constituents_norm.shape}")
    ...
    if verbose:
        print(f"Jets shape after selection: {jet_selection.shape}\nConstituents shape after selection: {constits_selection.shape}")

    # Convert to PyTorch tensors
    if verbose:
        print("Converting to PyTorch tensors...")
    evt_tensor, jet_tensor, constits_tensor = [helper.convert_to_tensor(data) for data in [events, jet_selection, constits_selection]]
    
    # Save tensors
    if verbose:
        print(f"Saving tensors to {output_prefix}_events.pt, {output_prefix}_jets.pt and {output_prefix}_constituents.pt...")
    ...
    if norm:
        if verbose:
            print(f"Saving normalization scalers to {output_prefix}_jet_scaler.pkl and {output_prefix}_constituent_scaler.pkl...")
    ...
    if verbose:
        print(f"Tensors saved to {output_prefix}_events.pt, {output_prefix}_jets.pt and {output_prefix}_constituents.pt")
        if norm:
            print(f"Normalization scalers saved to {output_prefix}_jet_scaler.pkl and {output_prefix}_constituent_scaler.pkl")
